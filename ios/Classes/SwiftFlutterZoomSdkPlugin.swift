...
public class SwiftFlutterZoomSdkPlugin: NSObject, FlutterPlugin{
	...
	public static func register(with registrar: FlutterPluginRegistrar) {
		...
		registrar.register(factory, withId: "flutter_zoom_sdk")
	}
}
...
public class ZoomView: NSObject, FlutterPlatformView, MobileRTCMeetingServiceDelegate, FlutterStreamHandler {
	...
	init(_ frame: CGRect, viewId: Int64, messenger: (NSObjectProtocol & FlutterBinaryMessenger)?, args: Any?) {
		self.frame = frame
		self.viewId = viewId
		self.channel = FlutterMethodChannel(name: "com.evilratt/flutter_zoom_sdk", binaryMessenger: messenger!)
		self.authenticationDelegate = AuthenticationDelegate()
		self.statusEventChannel = FlutterEventChannel(name: "com.evilratt/zoom_event_stream", binaryMessenger: messenger!)
	...
	public func joinMeeting(call: FlutterMethodCall, result: FlutterResult) {
		...
		if meetingService != nil {
			...
			meetingSettings?.meetingParticipantHidden = parseBoolean(data: arguments["noParticipantsButton"]!, defaultValue: false)
			...
			joinParam.noAudio = parseBoolean(data: arguments["noAudio"]!, defaultValue: false);
			joinParam.password = arguments["meetingPassword"]!!;
			joinParam.meetingNumber = arguments["meetingId"]!!;
			joinParam.userName = arguments["userId"]!!;
			
			let response = meetingService?.joinMeeting(with: joinParam)
	...
	private func getStateMessage(_ state: MobileRTCMeetingState?) -> [String] {
		...
		switch state {
			...
			case .webinarDePromote:
				...
				break
			case .ended:
				message = ["MEETING_STATUS_ENDED", "Meeting ended"]
				break
			case .failed:
				message = ["MEETING_STATUS_FAILED", "Meeting failed to start"]
				break;
			default:
